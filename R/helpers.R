...
choose_params <- function(X, type) {
  ...
  if (NCOL(X) == 1) {
    ...
    if (N < int1) {
      ...
      Gmodboost <- readRDS(system.file(paste0("choose_q_F_N=100_", type, ".rds"), package = "ddfs"))
    } else if (N >= int1 && N < int2) {
      # Gmodboost <- readRDS(paste0("/users/addj700/archive/ddfs/R/inst/choose_q_F_N=1000_", type, ".rds"))
      Gmodboost <- readRDS(system.file(paste0("choose_q_F_N=1000_", type, ".rds"), package = "ddfs"))
    } else if (N >= int2) {
    ...
    if (q_F == 1) {
      if (N < int1) {
        ...
        Gmodboost <- readRDS(system.file(paste0("choose_phi_F_1_N=100_", type, ".rds"), package = "ddfs"))
      } else if (N >= int1 && N < int2) {
        # Gmodboost <- readRDS(paste0("/users/addj700/archive/ddfs/R/inst/choose_phi_F_1_N=1000_", type, ".rds"))
        Gmodboost <- readRDS(system.file(paste0("choose_phi_F_1_N=1000_", type, ".rds"), package = "ddfs"))
      } else if (N >= int2) {
        # Gmodboost <- readRDS(paste0("/users/addj700/archive/ddfs/R/inst/choose_phi_F_1_N=10000_", type, ".rds"))
        Gmodboost <- readRDS(system.file(paste0("choose_phi_F_1_N=10000_", type, ".rds"), package = "ddfs"))
      }

    } else if (q_F == 2) {
      if (N < int1) {
        # Gmodboost <- readRDS(paste0("/users/addj700/archive/ddfs/R/inst/choose_phi_F_2_N=100_", type, ".rds"))
        Gmodboost <- readRDS(system.file(paste0("choose_phi_F_2_N=100_", type, ".rds"), package = "ddfs"))
      } else if (N >= int1 && N < int2) {
        # Gmodboost <- readRDS(paste0("/users/addj700/archive/ddfs/R/inst/choose_phi_F_2_N=1000_", type, ".rds"))
        Gmodboost <- readRDS(system.file(paste0("choose_phi_F_2_N=1000_", type, ".rds"), package = "ddfs"))
      } else if (N >= int2) {
        # Gmodboost <- readRDS(paste0("/users/addj700/archive/ddfs/R/inst/choose_phi_F_2_N=10000_", type, ".rds"))
        Gmodboost <- readRDS(system.file(paste0("choose_phi_F_2_N=10000_", type, ".rds"), package = "ddfs"))
      }
    }
    ...
    if (N < int1) {
      ...
      tree <- readRDS(system.file(paste0("choose_bw_N=100_", type,"_biv.rds"), package = "ddfs"))
    } else {
      # tree <- readRDS(paste0("/users/addj700/archive/ddfs/R/inst/choose_bw_N=1000_", type,"_biv.rds"))
      tree <- readRDS(system.file(paste0("choose_bw_N=1000_", type,"_biv.rds"), package = "ddfs"))
    }
    bw <- as.character(predict(tree, newdata = feat_X, type = "class"))
    ...
    if (N < int1) {
      ...
      Gmodboost <- readRDS(system.file(paste0("choose_q_F_N=100_", type,"_biv.rds"), package = "ddfs"))
    } else {
      # Gmodboost <- readRDS(paste0("/users/addj700/archive/ddfs/R/inst/choose_q_F_N=1000_", type,"_biv.rds"))
      Gmodboost <- readRDS(system.file(paste0("choose_q_F_N=1000_", type,"_biv.rds"), package = "ddfs"))
    }
    ...
    if (q_F == 1) {
      if (N < int1) {
        ...
        Gmodboost <- readRDS(system.file(paste0("choose_phi_F_1_N=100_", type,"_biv.rds"), package = "ddfs"))
      } else {
        # Gmodboost <- readRDS(paste0("/users/addj700/archive/ddfs/R/inst/choose_phi_F_1_N=1000_", type,"_biv.rds"))
        Gmodboost <- readRDS(system.file(paste0("choose_phi_F_1_N=1000_", type,"_biv.rds"), package = "ddfs"))
      }

    } else if (q_F == 2) {
      if (N < int1) {
        # Gmodboost <- readRDS(paste0("/users/addj700/archive/ddfs/R/inst/choose_phi_F_2_N=100_", type,"_biv.rds"))
        Gmodboost <- readRDS(system.file(paste0("choose_phi_F_2_N=100_", type,"_biv.rds"), package = "ddfs"))
      } else {
        # Gmodboost <- readRDS(paste0("/users/addj700/archive/ddfs/R/inst/choose_phi_F_2_N=1000_", type,"_biv.rds"))
        Gmodboost <- readRDS(system.file(paste0("choose_phi_F_2_N=1000_", type,"_biv.rds"), package = "ddfs"))
      }
    }
    phi_F <- predict(Gmodboost, feat_X, n = 4)
    phi_F <- max(min(phi_F, 1 - 1e-8), 1e-8) # make sure it is in (0,1)
    ...
    if (N < int1) {
      ...
      Gmodboost <- readRDS(system.file(paste0("choose_beta_N=100_", type,"_biv.rds"), package = "ddfs"))
    } else {
      # Gmodboost <- readRDS(paste0("/users/addj700/archive/ddfs/R/inst/choose_beta_N=1000_", type,"_biv.rds"))
      Gmodboost <- readRDS(system.file(paste0("choose_beta_N=1000_", type,"_biv.rds"), package = "ddfs"))
    }
