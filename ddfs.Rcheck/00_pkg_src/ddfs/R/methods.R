...
#' ddfs model. Above two dimensions, coefficients have a \code{basis.index} attribute
#' identifying the tensor indices, with the last dimension varying fastest.
#'
#' @details
...
#' @details For fits above two dimensions, supply a numeric matrix or data frame
#' with one observation per row and one column per fitted dimension. Named
#' columns must match the fitted dimension names and are reordered automatically;
#' unnamed columns use the training order. Missing values are not accepted.
#' Density is zero outside the fitted rectangle. CDF coordinates are clamped to
#' their fitted ranges, including infinite coordinates, so upper-bound coordinates
#' evaluate the corresponding marginal CDF. Quantiles and random sampling are
#' available only for univariate fits.
#'
#' @return
#' A numeric vector:
#' \itemize{
...
predict.ddfs <- function(object, newdata, fit = "pdf",...)
  ...
  if (!inherits(object, "ddfs")) stop("'object' must be of class 'ddfs'.")
  fit <- match.arg(fit, c("pdf", "cdf", "qf"))

  if (object$type == "Multiv - DDFS") {
    if (fit == "qf") stop("Quantile prediction is only available for univariate ddfs fits.")
    component <- if (fit == "pdf") object$f_hat else object$F_hat
    if (missing(newdata)) return(as.numeric(component$pred))
    coordinates <- as.matrix(newdata)
    if (!is.numeric(coordinates)) stop("'newdata' must be numeric.")
    if (NCOL(coordinates) != length(object$dimensions)) {
    ...
    if (!is.null(colnames(coordinates))) {
      if (anyDuplicated(colnames(coordinates)) ||
          !setequal(colnames(coordinates), object$dimensions)) {
        stop("'newdata' column names must match the fitted dimensions.")
      }
      coordinates <- coordinates[, object$dimensions, drop = FALSE]
    }
    if (anyNA(coordinates)) stop("'newdata' must not contain missing values.")
    if (!NROW(coordinates)) return(numeric())
    outside <- rep(FALSE, NROW(coordinates))
    for (j in seq_along(object$dimensions)) {
      bounds <- range(component$knots[[j]])
      outside <- outside | coordinates[, j] < bounds[1L] |
        coordinates[, j] > bounds[2L]
      coordinates[, j] <- pmax(bounds[1L], pmin(bounds[2L], coordinates[, j]))
    }
    basis <- ddfs_tensor_product(ddfs_basis_matrices(
      coordinates, component$knots, component$order
    ))
    values <- as.numeric(basis %*% component$coef)
    if (fit == "pdf") values[outside] <- 0
    return(values)
  }
  ...
  if (object$type == "Univ - DDFS") {
    ...
    if (missing(newdata)) {
      if (fit == "pdf") return(as.numeric(object$f_XY_hat$pred))
      if (fit == "cdf") return(as.numeric(object$F_XY_hat$pred))
      stop("Quantile prediction is only available for univariate ddfs fits.")
    }
    if (NCOL(newdata) != 2L) {
...
#' spline model. For multivariate fits this is a named list of knot vectors,
#' one per dimension.
#'
#' @details
...
knots.ddfs <- function(Fn, fit = c("pdf", "cdf"), options = c("all","internal"), ...) {
  ...
  if (!inherits(Fn, "ddfs")) stop("'Fn' must be of class 'ddfs'.")
  ...
  if (options == "internal") {
    ...
    if (Fn$type == "Multiv - DDFS") {
      kn <- lapply(kn, function(k) {
        k[seq_along(k) > depth & seq_along(k) <= length(k) - depth]
      })
    } else {
...
#' @name plot.ddfs
#' @description
#' Plot method for ddfs objects. Plots univariate and bivariate ddfs fits.
#' Direct plotting above two dimensions is not supported.
...
plot.ddfs <- function(x, f = NULL, main = NULL, legend.pos = NULL,
  ...
  if (x$type == "Multiv - DDFS") {
    stop("Direct plotting is not available above two dimensions.")
  }
...
print.ddfs <- function(x,
  ...
  if (x$type == "Multiv - DDFS") {
    ...
    cat("Number of internal knots: ", sum(lengths(kn)), "\n", sep = "")
    cat(paste(names(kn), lengths(kn), sep = "=", collapse = ", "))
  } else {
...
#' @examples
#' if (requireNamespace("SMPracticals", quietly = TRUE)) {
