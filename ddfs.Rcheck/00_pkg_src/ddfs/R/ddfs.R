...
#' function supports univariate, bivariate, and higher-dimensional joint density
#' estimation. Fits with two or more dimensions require the multidimensional
#' version of GeDS. Above two dimensions, omitted or \code{NULL} tuning parameters
#' use provisional defaults: \code{min.intknots = 10}, \code{beta = 0.1},
#' \code{phi_F = 0.95}, and \code{q_F = 2}. Explicit values take precedence.
#' These are fixed defaults, not automatic tuning, and are based on preliminary
#' 3D Gaussian experiments. The density order defaults to \code{n = 4}; its
#' computational cost grows rapidly with dimension. Univariate and bivariate
#' fits retain automatic selection of missing tuning parameters.
#' Only \code{stoptype = "RDMD"} and \code{schoenberg = FALSE} are supported
#' above two dimensions. These fits store \code{f_hat}, \code{F_hat}, and
#' \code{dimensions}, with a named knot list for each spline component.
#' @param max.coef Maximum number of coefficients in either tensor-product
#' spline for fits with two or more dimensions. Ignored for univariate fits.
#'
#' @param data Numeric vector, matrix or data frame, with observations in rows
...
#' that is, of internal knots in the final \code{ddfs} fits. If omitted, selected
#' automatically in one or two dimensions; defaults to 10 above two dimensions.
...
#' procedure (see Kaishev et al., 2016). If omitted, selected automatically in
#' one or two dimensions; defaults to \code{0.1} above two dimensions.
#' @param phi_F numeric parameter in the interval \eqn{(0,1)}
#' specifying the threshold for the ddfs iterations stopping rule. If omitted,
#' selected automatically in one or two dimensions; defaults to \code{0.95}
#' above two dimensions.
#' @param q_F Integer parameter that fine-tunes the stopping rule in the \code{ddfs}
#' fitting procedure. If omitted, selected automatically in one or two dimensions;
#' defaults to \code{2L} above two dimensions.
...
#' @param boundary_extension Non-negative numeric scalar or length-two vector
#' used only for univariate fits. The spline support is extended beyond the left
#' and right sample extremes by these multiples of \code{stats::bw.nrd0(data)}.
#' A scalar applies to both sides; zero preserves the observed-range support.
#' @param tails_count_threshold Numeric threshold used for fits with two or more dimensions.
...
#' each iteration. Default is \code{FALSE}. Direct plotting is not supported
#' above two dimensions.
...
#' }
#'
#' @return A fitted ddfs object containing the results of a univariate or multivariate
...
#'   Each contains predictions, a named list of knot vectors, coefficients,
#'   spline order, and dimension names. Coefficients carry a \code{basis.index}
#'   attribute with the last dimension varying fastest.}
...
#'   \code{"Biv - DDFS"} for bivariate ddfs estimation, or \code{"Multiv - DDFS"}
#'   for higher-dimensional fits. Bivariate components are named
#'   \code{f_XY_hat} and \code{F_XY_hat}.}
#'
...
ddfs <- function(data, n = 4L, min.intknots = NULL, max.intknots = 40L,
                 ...
                 stop_plotting = 0L, schoenberg = FALSE, max.coef = 100000L) {
  ...
  if (NCOL(data) > 2L) {
    if (length(missing)) {
      defaults <- list(min.intknots = 10L, beta = 0.1, phi_F = 0.95, q_F = 2L)
      list2env(defaults[missing], envir = environment())
      missing <- character()
    }
    if (!identical(stoptype, "RDMD") || !identical(schoenberg, FALSE)) {
      stop("Above two dimensions use 'stoptype = \"RDMD\"' and 'schoenberg = FALSE'.")
    }
    if (length(n) != 1L || is.na(n) || !(n %in% 2:4)) {
      stop("Above two dimensions 'n' must be 2, 3, or 4.")
    }
  }
  if (NCOL(data) >= 2L) {
    if (!exists("stageALoopND", asNamespace("GeDS"), inherits = FALSE) ||
        !exists("tensorProdND", asNamespace("GeDS"), inherits = FALSE)) {
      stop("Fits with two or more dimensions require the multidimensional version of GeDS.")
    }
  }
  ...
  if (!is.numeric(min.intknots) || length(min.intknots) != 1 ||
      min.intknots != as.integer(min.intknots) || min.intknots < 1)
  ...
  if (!is.numeric(boundary_extension) ||
      !(length(boundary_extension) %in% c(1L, 2L)) ||
      anyNA(boundary_extension) || any(!is.finite(boundary_extension)) ||
      any(boundary_extension < 0)) {
    stop("'boundary_extension' must contain one or two finite non-negative numeric values.")
  }
  ...
  if (NCOL(data) == 1) {
    ...
    fit <- MultivariateDensityFitter(
      data, n = n, min_iterations = min_iterations, max_iterations = max_iterations,
      beta = beta, phi_F = phi_F, q_F = q_F,
      tails_count_threshold = tails_count_threshold, plot = plot,
      resids_plot = resids_plot, pdf = pdf, cdf = cdf,
      stop_plotting = stop_plotting, max.coef = max.coef
    )
    if (NCOL(data) == 2L) {
      # Keep the established public bivariate layout; the legacy fitter itself
      # remains unchanged and is used only as a regression-test reference.
      fit <- list(
        f_XY_hat = ddfs_as_bivariate_component(fit$f_hat),
        F_XY_hat = ddfs_as_bivariate_component(fit$F_hat),
        type = "Biv - DDFS",
        args = list(XY = data, ecdf = fit$args$ecdf, phi = phi_F,
                    q = q_F, beta = beta),
        model = fit$model,
        RSS = fit$RSS
      )
